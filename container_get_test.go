...
func TestGet(t *testing.T) {
	...
	for testNumber,test := range tests {
		...
		if err := container.Register(test.Name, test.Thing); nil != err {
			t.Errorf("For test #%d, received an error when trying to register something under name %q. Error: (%T) %v.", testNumber, test.Name, err, err)
			return
		}
	...
	for testNumber,test := range tests {
		thing, err := container.Get(test.Name)
		if nil != err {
			t.Errorf("For test #%d, received an error when trying to get something from name %q. Error: (%T) %v.", testNumber, err, err)
			return
		}

		if actual, expected := fmt.Sprintf("%T", thing), fmt.Sprintf("%T", test.Thing); expected != actual {
			t.Errorf("For test #%d, expected type of thing gotten to be %q, but actually was %q.", testNumber, expected, actual)
			return
		}
	}
}
